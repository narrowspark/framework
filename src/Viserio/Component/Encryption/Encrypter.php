...
final class Encrypter implements EncrypterContract
{
    ...
    private static function unpackMessageForDecryption(string $ciphertext): array
    {
        ...
        $nonce = \mb_substr(
            $ciphertext,
            SecurityContract::HEADER_VERSION_SIZE + SecurityContract::HKDF_SALT_LEN, // 36
        ...
        $encrypted = \mb_substr(
            $ciphertext,
            // 60:
            SecurityContract::HEADER_VERSION_SIZE + SecurityContract::HKDF_SALT_LEN + \SODIUM_CRYPTO_STREAM_NONCEBYTES,
            // $length - 124
            $length - (
                SecurityContract::HEADER_VERSION_SIZE +
                SecurityContract::HKDF_SALT_LEN +
