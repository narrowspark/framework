...
trait OptionsResolverTrait
{
     ...
     * and can perform mandatory option checks if class implements RequiresMandatoryConfig. If the
     * ProvidesDefaultConfig interface is implemented, these options must be overridden by the provided config.
     * If you want to allow configurations for more then one instance use RequiresConfigId interface.
     *
     * The \Viserio\Contract\Config\RequiresConfigId interface is supported.
     *
     * @param array|\ArrayAccess $config
     * @param null|string        $configId Config name, must be provided if factory uses RequiresConfigId interface
     *
     * @throws \Viserio\Contract\Config\Exception\InvalidArgumentException         If the $configId parameter is provided but factory does not support it
     * @throws \Viserio\Contract\Config\Exception\UnexpectedValueException         If the $config parameter has the wrong type
     * @throws \Viserio\Contract\Config\Exception\OptionNotFoundException          If no options are available
     * @throws \Viserio\Contract\Config\Exception\MandatoryOptionNotFoundException If a mandatory option is missing
     *
     * @return array
     */
     ...
     *
     * @throws \Viserio\Contract\Config\Exception\MandatoryOptionNotFoundException
     *
     * @return void
     */
     ...
     *
     * @throws \Viserio\Contract\Config\Exception\OptionNotFoundException
     * @throws \Viserio\Contract\Config\Exception\UnexpectedValueException
     *
    ...
    private static function getConfigurationDimensions(
        ...
        foreach ($dimensions as $dimension) {
            ...
            if (! isset($config[$dimension])) {
                if (! isset($interfaces[RequiresMandatoryConfigContract::class])
                    && isset($interfaces[ProvidesDefaultConfigContract::class])
                ) {
                    break;
                }
        ...
        foreach ($validators as $key => $values) {
            ...
            if (\is_array($values) && isset($values[0]) && \is_string($values[0])) {
                ...
                foreach ($values as $check) {
                    if ($hasError === false && (self::$defaultTypeMap[$check] ?? false)) {
     ...
     * @param array|\ArrayAccess $config
     *
     * @throws \Viserio\Contract\Config\Exception\InvalidArgumentException If deprecation message cant be found or
